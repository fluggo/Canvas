...
"\n"
"// tex_coord - Coordinates of current texel in the input frame's texture\n"
"varying vec2 tex_coord[" G_STRINGIFY(VIDEO_MAX_FILTER_INPUTS) "];\n"
"\n"
"// frame_coord - Coordinates of current pixel in real frame coordinates (not texels)\n"
"varying vec2 frame_coord;\n"
"\n"
"void main() {\n"
"    gl_Position = vec4(position * vec2(2.0, 2.0) + vec2(-1.0, -1.0), 0.0, 1.0);\n"
"    frame_coord = position * vec2(frame_size) + frame_offset - vec2(0.5, 0.5);\n"
"\n"
