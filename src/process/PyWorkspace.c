...
WorkspaceItem_set_source( py_obj_WorkspaceItem *self, PyObject *value, void *closure ) {
    ...
    if( !takeVideoSource( value, holder ) )
        return -1;
...
WorkspaceItem_update( py_obj_WorkspaceItem *self, PyObject *args, PyObject *kw ) {
    ...
    if( source ) {
        if( !takeVideoSource( source, holder ) )
            return NULL;
    }
...
static PyTypeObject py_type_WorkspaceItem = {
    ...
    0,            // ob_size
    "fluggo.media.process.WorkspaceItem",    // tp_name
...
void init_Workspace( PyObject *module ) {
    ...
    PyModule_AddObject( module, "Workspace", (PyObject *) &py_type_Workspace );
